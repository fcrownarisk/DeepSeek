...
import random
import time
from pyglet.gl.glu import gluPerspective  # Needed for 3D projection
...
def normalize(position):
    """Convert floating-point block coordinates to integer block grid."""
    x, y, z = position
    return (int(round(x)), int(round(y)), int(round(z)))

def sectorize(position):
    """Group blocks into sectors for efficient rendering."""
    x, y, z = normalize(position)
    return (x // SECTOR_SIZE, y // SECTOR_SIZE, z // SECTOR_SIZE)


# Cube & Texture Coordinate Generation
...
# Texture indices (assuming a 4x4 texture atlas)
...
class Model(object):
    def __init__(self):
        ...
        self.world = {}          # block position -> texture tuple
        self.shown = {}          # currently visible blocks (position -> texture)
        self._shown = {}         # position -> pyglet vertex list
        self.sectors = {}        # sector -> list of positions
        self.queue = []          # deferred actions
        self.initialize()

    def initialize(self):
        """Generate a flat world with some random hills."""
        n = 80
        s = 1
        y = 0
            ...
            for y_off in range(c, c + h):
                for x in range(a - s, a + s + 1):
                    for z in range(b - s, b + s + 1):
                        ...
                        if (x - 0) ** 2 + (z - 0) ** 2 < 5 ** 2:
                            continue
                        self.add_block((x, y_off, z), t, immediate=False)
                s -= d

    def hit_test(self, position, vector, max_distance=8):
        """Raycast to find the block under the crosshair."""
        m = 8
        x, y, z = position
        dx, dy, dz = vector
        previous = None
...
class Window(pyglet.window.Window):
    def __init__(self, *args, **kwargs):
        ...
        self.model = Model()
        self.strafe = [0, 0]
        self.position = (0, 10, 0)   # Start a bit above ground
        self.rotation = (0, 0)
        self.sector = None
        self.reticle = None
        self.dy = 0
    ...
    def load_texture(self):
        ...
        try:
            img = pyglet.image.load(texture_file)
        except Exception as e:
            print(f"Warning: Could not load '{texture_file}'. Using checkerboard fallback.")
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        if self.exclusive:
            ...
            if (button == mouse.RIGHT) or \
                ...
                if texture != STONE:  # Prevent breaking bedrock
